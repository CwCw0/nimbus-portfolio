...
export default function PackageSection({ price, priceUsd }: { price: string; priceUsd: string }) {
  return (
    <section className="w-full px-16 py-24 max-md:px-6 max-md:py-16" style={{ background: "#0A0A0E" }}>
      <div className="mx-auto max-w-250">
        <div className="mb-16 flex items-center gap-4">
          <span className="font-body text-[10px] font-medium tracking-[4px] text-(--color-accent)">WHAT YOU GET</span>
        ...
        <div className="grid grid-cols-3 gap-5 max-lg:grid-cols-2 max-md:grid-cols-1">
          {packageItems.map((item) => {
            ...
            return (
              <div key={item.title} className="flex gap-4 border border-(--color-border) bg-(--color-bg-card) p-6 transition-all duration-300 hover:border-(--color-accent-border)">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center bg-(--color-accent-subtle)">
                  <Icon className="h-5 w-5 text-(--color-accent)" />
                </div>
                <div>
                  <h4 className="font-body text-[15px] font-semibold text-(--color-text-primary) mb-1">{item.title}</h4>
                  <p className="font-body text-[13px] leading-[1.6] text-(--color-text-dim)">{item.desc}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-12 flex flex-wrap items-center gap-3">
          <span className="font-body text-[10px] font-medium tracking-[3px] text-(--color-text-muted) mr-2">BUILT WITH</span>
          {techStack.map((tech) => (
            <span key={tech} className="border border-(--color-border) px-3 py-1.5 font-body text-[11px] text-(--color-text-muted)">
              {tech}
            </span>
          ))}
        </div>
        ...
        {/* Price + CTA */}
        <div className="mt-16 flex items-center justify-between border-t border-(--color-border) pt-12 max-md:flex-col max-md:gap-8 max-md:items-start">
          <div>
            <span className="font-display text-[40px] text-(--color-text-primary)">{price}</span>
            <span className="font-body text-sm text-(--color-text-muted) ml-3">{priceUsd}</span>
            <p className="font-body text-[13px] text-(--color-text-dim) mt-2">One-time purchase. Lifetime updates. No subscription.</p>
          </div>
        ...
        {/* Guarantee */}
        <div className="mt-8 flex items-start gap-3 border border-(--color-border) bg-(--color-bg-card) p-5 max-w-160">
          <Check className="h-5 w-5 shrink-0 text-(--color-accent) mt-0.5" />
          <div>
            <span className="font-body text-[14px] font-semibold text-(--color-text-primary)">14-day refund guarantee</span>
            <p className="font-body text-[13px] text-(--color-text-dim) mt-1">If the template doesn&apos;t work as described, email us for a full refund. No questions.</p>
          </div>
        </div>
      </div>
    </section>
