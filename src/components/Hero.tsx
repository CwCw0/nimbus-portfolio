  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 max-md:hidden">
        {/* Ambient glow */}
        <div
          className="absolute left-[400px] top-[50px] h-[650px] w-[900px] rounded-full opacity-40"
          style={{
            background:
              "radial-gradient(circle, #7C5CFC0A 0%, #7C5CFC04 50%, transparent 100%)",
          }}
        />
        {/* Blob core */}
        <div
          className="absolute left-[850px] top-[150px] h-[420px] w-[460px] rounded-full opacity-20"
          style={{
            background:
              "radial-gradient(circle, #7C5CFC30 0%, #7C5CFC18 50%, transparent 100%)",
          }}
        />
        {/* Blob layer 2 */}
        <div
          className="absolute left-[920px] top-[230px] h-[300px] w-[340px] rotate-[15deg] rounded-full opacity-35"
          style={{
            background:
              "radial-gradient(circle, #7C5CFC20 0%, #7C5CFC0C 50%, transparent 100%)",
          }}
        />
        {/* Blob highlight */}
        <div
          className="absolute left-[1050px] top-[330px] h-[120px] w-[140px] rounded-full opacity-50"
          style={{
            background:
              "radial-gradient(circle, #7C5CFC30 0%, #7C5CFC10 40%, transparent 100%)",
          }}
        />
        {/* Secondary blob */}
        <div
          className="absolute left-[200px] top-[500px] h-[240px] w-[280px] rounded-full opacity-10"
          style={{
            background:
              "radial-gradient(circle, #7C5CFC0C 0%, #7C5CFC05 50%, transparent 100%)",
          }}
        />
        {/* Grid lines */}
      ...
      <div className="relative z-10 flex h-full w-full flex-col items-center gap-10 px-16 pb-10 pt-[100px] max-md:px-6 max-md:pt-20 max-md:pb-16 max-md:gap-8">
        ...
        <div className="hero-animate flex items-center gap-2 rounded-full border border-[var(--color-border-light)] px-5 py-2">
          ...
          <span className="font-inter text-[11px] tracking-[1px] text-[var(--color-text-muted)]">
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <p className="hero-animate w-[600px] text-center font-inter text-base leading-[1.6] text-[var(--color-text-dim)] max-md:w-full max-md:text-sm">
            Nimbus is a creative studio specializing in websites, branding,
            UI/UX, SEO
            <br />
            and AI-powered tools for freelancers, startups and growing
            businesses.
          </p>
        </div>
        ...
        <div className="hero-animate flex items-center gap-4 max-md:flex-col max-md:w-full">
          <a
            href="#contact"
            className="flex items-center gap-2.5 bg-gradient-to-br from-[var(--color-accent)] to-[var(--color-accent-light)] px-8 py-4 font-inter text-[15px] font-semibold text-[#0A0A0B] transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_0_24px_#7C5CFC20] max-md:w-full max-md:justify-center"
          >
            Start a Project
          ...
          <a
            href="#work"
            className="flex items-center gap-2.5 border border-[var(--color-border-light)] px-8 py-4 font-inter text-[15px] font-medium text-[var(--color-text-secondary)] transition-all duration-300 hover:border-[var(--color-accent-border)] hover:text-white max-md:w-full max-md:justify-center"
          >
            View Work
          </a>
        </div>
        ...
        <div className="hero-animate flex w-full items-center gap-9 pt-10 max-md:hidden">
          <span className="font-inter text-[10px] font-medium tracking-[2px] text-[var(--color-text-subtle)]">
            Trusted by
          </span>
          <div className="h-4 w-px bg-[#2A2A2E]" />
          {["FLOWSTATE", "ARCLINE", "VEKTOR", "ONSHIFT", "BASECAMP"].map(
            (brand) => (
              <span
                key={brand}
                className="font-inter text-[11px] font-semibold tracking-[3px] text-[var(--color-text-subtle)]"
              >
                {brand}
              </span>
            )
